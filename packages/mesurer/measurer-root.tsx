...
import { MEASURER_COMMAND_EVENT } from "./commands";
...
const getDefaultStorage = (persistOnReload: boolean) => {
  if (!persistOnReload || typeof window === "undefined") return null;
...
function MeasurerClient({
  ...
  storage,
  commandTarget,
}: Required<Pick<MeasurerProps, "highlightColor" | "guideColor" | "hoverHighlightEnabled" | "persistOnReload">> &
  Pick<MeasurerProps, "initialToolMode"> &
  Pick<MeasurerRootProps, "initialEnabled" | "styleTarget" | "storage" | "commandTarget"> & {
    portalTarget: MeasurerPortalTarget;
  }) {
  ...
  useIsomorphicLayoutEffect(() => {
    return ensureMeasurerStyles(MESURER_STYLES, styleTarget);
  }, [styleTarget]);

  const persistedState = useMemo(() => {
    if (!persistOnReload || !storage) return null;
  ...
  const {
    ...
    initialEnabled: persistedState?.enabled ?? initialEnabled,
    initialToolMode: persistedState?.toolMode ?? initialToolMode,
  });
  ...
  useEffect(() => {
    if (!persistOnReload || !storage || !persistPayload) return;

    try {
      storage.setItem(DEFAULT_STORAGE_KEY, persistPayload);
    } catch {
  ...
  }, [persistOnReload, persistPayload, storage]);
  ...
  useEffect(() => {
    ...
    commandTarget.addEventListener(
      MEASURER_COMMAND_EVENT,
      handleCommand as EventListener,
    );

    return () => {
      commandTarget.removeEventListener(
        MEASURER_COMMAND_EVENT,
        handleCommand as EventListener,
      );
    };
...
export function MeasurerRoot({
  ...
  const resolvedStorage = useMemo(
    () => (storage === undefined ? getDefaultStorage(persistOnReload) : storage),
    [persistOnReload, storage],
  );

  if (!hydrated || !resolvedPortalTarget) return null;
  ...
  return (
    <MeasurerClient
      ...
      styleTarget={resolvedStyleTarget}
